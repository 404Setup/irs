...
Configure the Shadow plugin to relocate the `one.tranic` package. This ensures that the Irs library’s classes do not conflict with other plugins.
...
To ensure that relocation works as intended, decompile the built jar and verify that the `one.tranic` package has been renamed to `your.plugin.shadow.one.tranic` or your specified relocation path.
